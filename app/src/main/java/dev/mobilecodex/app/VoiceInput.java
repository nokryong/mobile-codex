...
final class VoiceInput {
    ...
    static JSONObject start(Context context, String origin, JSONObject draft) throws Exception {
        ...
        if (active()) throw new IOException("이미 음성 입력 중입니다. 인식을 마치거나 취소해 주세요.");
        if ((!origin.equals("main") && !origin.equals("floating")) || draft.optString("scope").isEmpty()) throw new IOException("입력할 대화를 먼저 열어 주세요.");
        if (draft.optString("original").length() > 100000) throw new IOException("초안이 너무 깁니다. 내용을 나눠서 입력해 주세요.");
    ...
    static void complete(Context context, JSONObject request, String text, String error) throws Exception {
        ...
        if (!receipts(context).edit().putString(request.getString("receiptId"), result.toString()).commit()) throw new IOException("음성 입력 결과를 저장하지 못했습니다.");
    ...
    static void acknowledge(Context context, String origin, String id) throws IOException {
        ...
        if (!saved.isEmpty() && origin.equals(parse(saved).optString("origin")) && !receipts(context).edit().remove(id).commit()) throw new IOException("음성 입력 확인 기록을 저장하지 못했습니다.");
    }
