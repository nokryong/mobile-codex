...
public final class DocumentStore {
    ...
    public synchronized JSONObject select(Uri uri, String projectKey) throws Exception {
        ...
        if (!selected.directory()) throw new IOException("폴더가 아닙니다.");
    ...
    public synchronized JSONObject selectProject(String key) throws Exception {
        if (projects.removed(key)) { detachedKey = key; detachedName = "연결 해제된 프로젝트"; tree = null; label = detachedName; return workspace(); }
        detachedKey = ""; detachedName = "";
        projects.select(key);
    ...
    public synchronized JSONObject removeProject(String key) throws IOException {
        ...
        if (!context.getSharedPreferences("projects", 0).edit().putString("registry", staged.toJson()).commit())
            throw new IOException("프로젝트 목록 변경을 저장하지 못했습니다.");
        projects = staged;
    ...
    public synchronized void requireWorkspaceAvailable() throws IOException {
        if (!detachedKey.isBlank()) throw new IOException("연결 해제된 프로젝트 대화입니다. 폴더를 다시 연결하거나 일반 대화에서 새 작업을 시작해 주세요.");
        ProjectRegistry.Project project = projects.selected();
        if (project != null) {
            try {
                if (tree == null || !available(project) || !query(documentUri(tree)).directory())
                    throw new IOException("작업 폴더를 열 수 없습니다.");
            } catch (Exception e) {
                tree = null;
                throw new IOException("이 프로젝트 폴더에 접근할 수 없습니다. 프로젝트에서 폴더를 다시 연결해 주세요.", e);
            }
        }
    }
    ...
    synchronized JSONObject mention(String path, AttachmentStore attachments) throws Exception {
        ...
        if (node.directory()) throw new IOException("폴더는 대화에 첨부할 수 없습니다.");
    ...
    synchronized JSONObject image(String path, ImageStore images) throws Exception {
        Node n = resolve(path);
        if (n.directory()) throw new IOException("이미지 파일을 선택해 주세요.");
        try (InputStream in = context.getContentResolver().openInputStream(n.uri)) {
            if (in == null) throw new IOException("이미지 파일을 읽을 수 없습니다.");
    ...
    private void requireTree() throws IOException {
        ...
        if (tree == null) throw new IOException("먼저 작업 폴더를 선택해 주세요.");
    }
    private Node query(Uri uri) throws IOException {
        try (Cursor c = context.getContentResolver().query(uri, COLUMNS, null, null, null)) {
            if (c == null || !c.moveToFirst()) throw new FileNotFoundException("파일을 찾을 수 없습니다.");
    ...
    private List<Node> children(Node parent) throws IOException {
        if (!parent.directory()) throw new IOException("폴더가 아닙니다.");
        ...
        try (Cursor c = context.getContentResolver().query(children, COLUMNS, null, null, null)) {
            if (c == null) throw new IOException("폴더를 열 수 없습니다.");
            while (c.moveToNext()) {
                if (result.size() >= 5000) throw new IOException("한 폴더의 항목이 5,000개를 초과합니다. 더 작은 폴더를 선택해 주세요.");
        ...
        for (String segment : WorkspacePath.segments(path)) {
            ...
            for (Node child : children(n)) {
                if (child.name.equals(segment)) {
                    if (found != null) throw new IOException("이름이 같은 항목이 여러 개입니다: " + segment);
                    found = child;
                }
            }
            if (found == null) throw new FileNotFoundException("파일을 찾을 수 없습니다: " + path);
            n = found;
        }
        return n;
    }
    ...
    public synchronized JSONObject search(String text) throws IOException {
        if (text == null || text.isBlank()) throw new IllegalArgumentException("검색어를 입력해 주세요.");
    ...
    private byte[] bytes(Node n, int limit) throws IOException {
        if (n.directory()) throw new IOException("폴더는 텍스트로 열 수 없습니다.");
        if (n.size > limit) throw new IOException("파일 크기 제한을 초과했습니다 (" + (limit / 1024 / 1024) + " MiB).");
        try (InputStream in = context.getContentResolver().openInputStream(n.uri); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            while ((count = in.read(buffer)) != -1) {
                if (out.size() + count > limit) throw new IOException("파일 크기 제한을 초과했습니다.");
    ...
    private String decode(byte[] data) throws IOException {
        try {
            ...
            if (value.indexOf('\0') >= 0) throw new IOException("텍스트 파일이 아닙니다.");
            return value;
        } catch (CharacterCodingException e) { throw new IOException("UTF-8 텍스트 파일만 편집할 수 있습니다."); }
    }
    ...
    public synchronized Mutation prepare(String operation, JSONObject args) throws Exception {
        ...
        if (path.isEmpty()) throw new IOException("작업 폴더 자체는 변경할 수 없습니다.");
        Node n = null;
        byte[] before = null;
        String title;
        String preview = path;
        if (operation.equals("mobile_create") || operation.equals("mobile_mkdir")) {
            if (exists(path)) throw new IOException("같은 이름의 파일 또는 폴더가 이미 있습니다.");
            if (!resolve(WorkspacePath.parent(path)).directory()) throw new IOException("상위 폴더가 없습니다.");
            title = operation.equals("mobile_create") ? "파일 만들기" : "폴더 만들기";
        } else {
            n = resolve(path);
            title = switch (operation) {
                case "mobile_write" -> "파일 수정";
                case "mobile_delete" -> "파일 삭제";
                case "mobile_rename" -> "이름 변경";
                case "mobile_move" -> "파일 이동";
        ...
        if (operation.equals("mobile_create") || operation.equals("mobile_write")) {
            if (args.getString("content").getBytes(StandardCharsets.UTF_8).length > TEXT_LIMIT)
                throw new IOException("텍스트는 최대 1 MiB까지 저장할 수 있습니다.");
        }
        ...
        if (operation.equals("mobile_rename")) {
            ...
            if (exists(target)) throw new IOException("같은 이름의 항목이 이미 있습니다.");
            preview += "\n→ " + target;
        }
        if (operation.equals("mobile_move")) {
            String destination = args.getString("destination"); WorkspacePath.segments(destination);
            if (destination.equals(path) || destination.startsWith(path + "/")) throw new IOException("자기 안으로 이동할 수 없습니다.");
            if (!resolve(destination).directory()) throw new IOException("대상 폴더가 아닙니다.");
            if ((n.flags & Document.FLAG_SUPPORTS_MOVE) == 0) throw new IOException("이 저장소는 파일 이동을 지원하지 않습니다.");
            String target = destination.isEmpty() ? n.name : destination + "/" + n.name;
            if (exists(target)) throw new IOException("대상 폴더에 같은 이름의 항목이 있습니다.");
            preview += "\n→ " + target;
        }
        return new Mutation(operation, parse(args.toString()), tree.toString(), n == null ? "" : n.id,
            n == null ? 0 : n.modified, before, title, preview);
    }
    private String excerpt(String s) { return s.length() <= 6000 ? s : s.substring(0, 6000) + "\n… (미리보기 생략)"; }
    public synchronized JSONObject commit(Mutation m) throws Exception {
        requireTree();
        if (!tree.toString().equals(m.tree)) throw new IOException("작업 폴더가 변경되어 요청을 취소했습니다.");
        ...
        if (!now.documentId.equals(m.documentId) || now.modified != m.modified ||
            ...
            throw new IOException("확인하는 동안 파일이 변경되었습니다. 다시 시도해 주세요.");
        ...
        switch (m.operation) {
            case "mobile_create", "mobile_mkdir" -> {
                ...
                if (created == null) throw new IOException("생성하지 못했습니다.");
            ...
            case "mobile_delete" -> {
                if (!DocumentsContract.deleteDocument(context.getContentResolver(), resolve(path).uri)) throw new IOException("삭제하지 못했습니다.");
            }
            case "mobile_rename" -> {
                if (DocumentsContract.renameDocument(context.getContentResolver(), resolve(path).uri, m.args.getString("name")) == null)
                    throw new IOException("이름을 변경하지 못했습니다.");
            }
            case "mobile_move" -> {
                if (DocumentsContract.moveDocument(context.getContentResolver(), resolve(path).uri, parent.uri,
                    resolve(m.args.getString("destination")).uri) == null) throw new IOException("이동하지 못했습니다.");
            }
            default -> throw new IOException("지원하지 않는 작업입니다.");
        }
    ...
    private void write(Uri uri, byte[] data) throws IOException {
        try (OutputStream out = context.getContentResolver().openOutputStream(uri, "wt")) {
            if (out == null) throw new IOException("파일을 저장할 수 없습니다.");
            out.write(data); out.flush();
        }
        if (!Arrays.equals(bytes(query(uri), TEXT_LIMIT), data)) throw new IOException("저장한 내용을 검증하지 못했습니다. 복구 사본을 확인해 주세요.");
    }
    private String backup(Mutation m, String mime) throws Exception {
        String id = UUID.randomUUID().toString();
        if (backups.getUsableSpace() < m.before.length + 10 * 1024 * 1024L) throw new IOException("복구 사본을 저장할 공간이 부족합니다.");
    ...
    public synchronized void exportRecovery(String id, Uri destination) throws Exception {
        ...
        try (OutputStream out = context.getContentResolver().openOutputStream(destination, "wt")) {
            if (out == null) throw new IOException("복구 사본을 내보낼 수 없습니다.");
            out.write(data);
        }
    }
