...
public final class PhoneControlGate {
    ...
    public synchronized long ticket() {
        if (!enabled) throw new IllegalStateException("휴대폰 제어가 꺼져 있습니다. 설정 → 도구에서 직접 켜 주세요.");
        return generation;
    }
    public synchronized void check(long ticket) {
        if (!enabled || ticket != generation) throw new IllegalStateException("휴대폰 제어가 중지되었습니다. 이전 요청은 취소됩니다.");
    }
}
