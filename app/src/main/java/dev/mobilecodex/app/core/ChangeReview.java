...
public final class ChangeReview {
    ...
    public JSONObject list(File root) throws Exception {
        ...
        return obj("entries", entries, "note", "Git HEAD와 현재 작업 파일을 비교합니다. 직접 수정한 내용도 포함합니다. 복원은 파일 내용만 바꾸며 스테이징은 유지합니다.");
    }
    public JSONObject preview(File root, String path) throws Exception {
        ...
        if (!head.isEmpty()) {
            ...
            if (!tree.isEmpty()) {
                ...
                if (!fields[0].equals("100644") && !fields[0].equals("100755")) throw new IOException("일반 파일만 복원할 수 있습니다. 링크·하위 저장소는 터미널에서 확인해 주세요.");
                original = git(root, "cat-file", "blob", fields[2]);
            }
        }
        return remember(root, path, current, original, head, "HEAD 내용으로 복원");
    }
    private JSONObject remember(File root, String path, byte[] current, byte[] replacement, String head, String label) throws Exception {
        if ((current != null && current.length > LIMIT) || (replacement != null && replacement.length > LIMIT)) throw new IOException("8 MiB보다 큰 파일은 터미널에서 검토해 주세요.");
        ...
        boolean limited = before.startsWith("[바이너리 파일") || after.startsWith("[바이너리 파일") || before.startsWith("[큰 파일") || after.startsWith("[큰 파일");
        return obj("token", token, "path", path, "before", before, "after", after, "beforeExists", replacement != null,
            ...
            "note", "현재 파일은 복원 전에 별도로 보관합니다. 다른 곳에서 파일이 바뀌면 복원을 중단합니다. Git 스테이징은 변경하지 않습니다.");
    }
    public JSONObject restore(File root, String token) throws Exception {
        Preview p = previews.get(token);
        if (p == null || !p.root.equals(root.getCanonicalFile()) || System.currentTimeMillis() - p.time > 300000) throw new IOException("미리보기가 만료되었거나 프로젝트가 바뀌었습니다. 다시 열어 주세요.");
        Path target = safe(root, p.path);
        if (p.head != null && !p.head.equals(head(root))) throw new IOException("Git HEAD가 변경되었습니다. 변경 사항을 다시 확인해 주세요.");
        if (!Arrays.equals(read(target), p.expected)) throw new IOException("미리보기 이후 파일이 변경되었습니다. 덮어쓰지 않았습니다.");
        if (Arrays.equals(p.expected, p.replacement)) throw new IOException("복원할 변경 사항이 없습니다.");
        ...
        if (!Arrays.equals(read(safe(root, p.path)), p.expected)) throw new IOException("백업 중 파일이 변경되었습니다. 덮어쓰지 않았습니다.");
        if (p.replacement == null) Files.delete(target);
        else {
            if (!Files.isDirectory(target.getParent())) throw new IOException("상위 폴더가 없습니다. 먼저 폴더를 복원해 주세요.");
            ...
            try {
                ...
                if (!Arrays.equals(read(safe(root, p.path)), p.expected)) throw new IOException("파일이 변경되었습니다. 다시 확인해 주세요.");
                Files.move(stage, target, StandardCopyOption.REPLACE_EXISTING);
            } finally { Files.deleteIfExists(stage); }
        }
        if (!Arrays.equals(read(target), p.replacement)) throw new IOException("복원 내용을 검증하지 못했습니다. 복원 사본을 확인해 주세요.");
    ...
    public JSONObject previewBackup(File root, String id) throws Exception {
        if (!id.matches("[a-f0-9-]{36}")) throw new IOException("잘못된 복원 기록입니다.");
        JSONObject item = parse(Utf8Files.read(new File(backups, id + ".json").toPath()));
        if (!root.getCanonicalPath().equals(item.optString("root"))) throw new IOException("다른 프로젝트의 복원 기록입니다.");
        String path = item.getString("path"); byte[] current = read(safe(root, path));
        if (!item.optBoolean("completed")) throw new IOException("완료되지 않은 복원입니다. 보관된 사본은 터미널에서 확인해 주세요: " + new File(backups, id + ".bin"));
        if (!hash(current).equals(item.getString("expectedAfter"))) throw new IOException("복원 이후 파일이 다시 변경되었습니다. 자동으로 덮어쓰지 않습니다.");
        byte[] saved = item.getBoolean("existed") ? Files.readAllBytes(new File(backups, id + ".bin").toPath()) : null;
        if (!hash(saved).equals(item.getString("sha256"))) throw new IOException("복원 사본의 검증에 실패했습니다.");
        return remember(root, path, current, saved, null, "복원 전 내용으로 되돌리기");
    }
    ...
    private static String text(byte[] bytes) {
        ...
        if (bytes.length > 1024 * 1024) return "[큰 파일 · " + bytes.length + " bytes]";
        try {
            ...
            return text.indexOf('\0') < 0 ? text : "[바이너리 파일 · " + bytes.length + " bytes]";
        } catch (CharacterCodingException e) { return "[바이너리 파일 · " + bytes.length + " bytes]"; }
    }
    private static byte[] read(Path file) throws IOException {
        if (!Files.exists(file, LinkOption.NOFOLLOW_LINKS)) return null;
        if (!Files.isRegularFile(file, LinkOption.NOFOLLOW_LINKS) || Files.size(file) > LIMIT) throw new IOException("일반 파일(8 MiB 이하)만 미리보기에서 복원할 수 있습니다.");
        try (InputStream input = Files.newInputStream(file); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            while ((n = input.read(block)) != -1) { if (out.size() + n > LIMIT) throw new IOException("파일 크기가 변경되었습니다."); out.write(block, 0, n); }
            return out.toByteArray();
        }
    }
    private static Path safe(File root, String name) throws IOException {
        if (name.isEmpty() || name.startsWith("/") || name.indexOf('\0') >= 0) throw new IOException("잘못된 파일 경로입니다.");
        Path base = root.getCanonicalFile().toPath(), current = base;
        for (String part : name.split("/", -1)) {
            if (part.isEmpty() || part.equals(".") || part.equals("..") || part.equalsIgnoreCase(".git")) throw new IOException("작업 폴더 밖이나 Git 내부 파일은 복원할 수 없습니다.");
            current = current.resolve(part);
            if (Files.isSymbolicLink(current)) throw new IOException("링크를 따라 복원하지 않습니다.");
        }
        return current;
    }
}
